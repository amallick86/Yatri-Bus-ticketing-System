...
session_start();
	...
	$cser=mysqli_connect("localhost","root","","yatri_db");
					...
					$querya = "SELECT * from admin_account where cmp_name='$cmp_name'"; 
					$resulta = mysqli_query($cser, $querya) or die ( mysqli_error());
					$rowa = mysqli_fetch_assoc($resulta);
					$bus_uname = $rowa['admin_uname'];
	...
	{  
		...
		$status   = 1;
		$seatno   = 1;
		if($bus_seat =="Select a Seat *"){
	echo'<script language="javascript">';
	echo 'alert("First Choose Your Seat")';
	echo '</script>';}
	else{
	...
	if( $user_amount == 0){
	echo'<script language="javascript">';
	echo 'alert("You Have No Sufficient Balance")';
	echo '</script>';}
	else{
	...
	$query2 = "SELECT * from admin_account where cmp_name='".$cmp_name."'"; 
		...
		$result6 = mysqli_query($cser, "insert into user_transection_details set username='$username',name='$name',date='$bus_date',cmp_name='$cmp_name',bus_num='$bus_num',admin_uname='$bus_uname',seat='$seats',status='Paid',credit='$total' ");
		 if ($result6) {
...
<html lang="en">
  ...
  <body>
    ...
    <!-- Medium input -->
	<div class="md-form">
  <input type="text" id="inputMDEx" class="form-control"  name="phone" value="<?php echo $row["user_phn"];?>" READONLY>
...
<?php 
	$query3 = "SELECT * from bus_seats where bus_num ='$bus_num' and seat_date='$bus_date'and status='0'"; 
				...
				?>
<select class="browser-default custom-select" name="seats" style="color:blue;width:90%">
